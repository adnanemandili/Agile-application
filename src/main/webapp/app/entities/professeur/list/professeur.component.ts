...
import { EntityArrayResponseType, ProfesseurService } from '../service/professeur.service';
...
export class ProfesseurComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProfesseur): number => this.professeurService.getProfesseurIdentifier(item);
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.professeurs || this.professeurs.length === 0) {
            this.load();
          }
        }),
      )
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.professeurs = this.refineData(dataFromBody);
  }

  protected refineData(data: IProfesseur[]): IProfesseur[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IProfesseur[] | null): IProfesseur[] {
    return data ?? [];
  }
